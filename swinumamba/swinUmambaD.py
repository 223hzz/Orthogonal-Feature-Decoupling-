...
def init_weights(m):
    """
    简单的权重初始化函数 (Kaiming / He Initialization)
    有助于模型从零开始训练时更快收敛
    """
...
def get_swin_umamba_d(
        input_channels: int = 1,
        num_classes: int = 3,
        deep_supervision: bool = True
):
    """
    构建 SwinUMambaD 模型 (随机初始化，不加载任何预训练权重)
    """

    # 1. 定义 VSSM Encoder 配置 (Tiny Version)
    vss_args = dict(
        patch_size=4,
        in_chans=input_channels,
        depths=[2, 2, 9, 2],  # Tiny 版本标准深度
        dims=96,  # Tiny 版本标准维度
    ...
    # 4. 执行随机初始化
    # 注意：Mamba 内部的核心参数 (dt, A, D) 通常有自己的初始化逻辑，
    # 这里我们主要确保 Conv 和 Linear 层有良好的初始值。
    # VSSMEncoder 内部其实自带了 _init_weights，这里再次 apply 确保 Decoder 也被初始化
    model.apply(init_weights)

    print("SwinUMambaD created with random initialization (Scratch).")
    return model

# #使用
# import torch
# from SwinUMambaD import get_swin_umamba_d
#
# # === 实例化模型 (从零训练) ===
# model = get_swin_umamba_d(
#     input_channels=1,
#     num_classes=3,
#     deep_supervision=True
# ).cuda()

# 接下来正常定义 Optimizer 和 Loss 即可
...
#
#     # 1. 实例化模型
#     model = get_swin_umamba_d(input_channels=3,num_classes=2,deep_supervision=False).to(device)
#     model.eval()
#
#     # 2. 创建虚拟输入
