...
def get_swin_umamba(
        ...
        num_classes: int = 3,
        deep_supervision: bool = True,
        use_pretrain: bool = False,  # 默认 False (从零训练)
        pretrained_path: str = None  # 如果 use_pretrain=True，这里填路径
):
    """
    独立构建 SwinUMamba (Hybrid Version)
    无需 nnU-Net 环境即可运行
    """

    # 1. 实例化模型
    # 这里使用了默认的 feat_size 配置，对应 Tiny/Base 混合体
    model = SwinUMamba(
        in_chans=input_channels,
        out_chans=num_classes,
        feat_size=[48, 96, 192, 384, 768],  # 经典的 Swin-UNet/UNETR 维度设置
        deep_supervision=deep_supervision,
        hidden_size=768,  # 对应最底层的维度
        drop_path_rate=0.2
    )

    # 2. 权重初始化策略
    if use_pretrain and pretrained_path is not None:
        # 方案 A: 加载预训练权重
        print(f"Loading pretrained weights from: {pretrained_path}")
        try:
            # 复用文件中已有的 load_pretrained_ckpt 函数逻辑，但稍作修改以适应外部调用
            ckpt = torch.load(pretrained_path, map_location='cpu')
            model_dict = model.state_dict()

            # 兼容带 'model' key 或直接是 state_dict 的情况
            state_dict = ckpt['model'] if 'model' in ckpt else ckpt

            # 过滤逻辑 (排除不匹配的层，如 patch_embed 通道不同，或 head 类别不同)
            skip_params = ["head.weight", "head.bias", "out_layers"]

            for k, v in state_dict.items():
                if any(s in k for s in skip_params): continue

                # 修正 VSSM Encoder 的前缀 (如果预训练权重是纯 VSSM 的)
                kr = f"vssm_encoder.{k}"

                # 处理 downsample 命名不匹配的问题 (layers.x.downsample -> downsamples.x)
                if "downsample" in kr:
                    i_ds = int(re.findall(r"layers\.(\d+)\.downsample", kr)[0])
                    kr = kr.replace(f"layers.{i_ds}.downsample", f"downsamples.{i_ds}")
                ...
                if kr in model_dict and v.shape == model_dict[kr].shape:
                    ...
                    # 尝试直接匹配 (针对非 encoder 部分)
                    model_dict[k] = v

            model.load_state_dict(model_dict)
            print("Pretrained weights loaded (partial match).")
        ...
        # 方案 B: 从零训练 (Random Init)
        print("Initializing weights from scratch (Kaiming Init)...")
        model.apply(init_weights)
...
# ==========================================
#  测试区域 (Python SwinUMamba.py 直接运行)
# ==========================================
if __name__ == "__main__":
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Running SwinUMamba (Hybrid) on {device}")

    # 1. 构建模型 (模拟 CT 输入 -> 3类分割)
    model = get_swin_umamba(
        input_channels=1,
        num_classes=3,
        deep_supervision=True,
        use_pretrain=False  # 设为 False 以测试随机初始化
    ).to(device)
    ...
    try:
        ...
        if isinstance(output, (list, tuple)):
            print(f"✅ Deep Supervision Output (List of {len(output)})")
...
# 使用
# from SwinUMamba import get_swin_umamba
#
# # 实例化
# model = get_swin_umamba(
#     input_channels=1,
...
#
#     # 1. 实例化模型
#     model = get_swin_umamba(input_channels=3,num_classes=2,deep_supervision=False, use_pretrain=False).to(device)
#     model.eval()
#
#     # 2. 创建虚拟输入
