...
def dcswin_small(pretrained=False, num_classes=2, weight_path=''):
...
def dcswin_tiny(pretrained=False, num_classes=2, weight_path=''):
    model = DCSwin(encoder_channels=(96, 192, 384, 768),
                   num_classes=num_classes,
                   embed_dim=96,
                   depths=(2, 2, 6, 2),
...
def measure_model():
    ...
    # 1. 实例化模型
    model =dcswin_small(num_classes=2).to(device)
    model.eval()
