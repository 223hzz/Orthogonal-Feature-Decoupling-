...
from nets.omnisr.layernorm import LayerNorm2d
...
inputs = torch.randn(1, 128, 14, 14)
model1 = Channel_Attention(dim=128,heads=4,bias=False,dropout=0.,window_size=14)
# model2 = Attention(dim=128,dim_head=32,dropout=0.,window_size=14,with_pe=True)
# x = rearrange(inputs, 'b c h w -> b 1 1 h w c')  # → shape: (1, 1, 1, 14, 14, 128)
# flops = FlopCountAnalysis(model2, x)
flops = FlopCountAnalysis(model1, inputs)
...
# model = Channel_Attention(dim=128,heads=4,bias=False,dropout=0.,window_size=16).to(device) # 加载你的模型
# model.eval()  # 切换到评估模式
#
# # 准备一个符合尺寸的随机输入张量
# # Batch size通常设为1来测量纯粹的延迟
# input_tensor = torch.randn(1, 128, 16, 16).to(device)
# # 2. 预热 (Warm-up)
