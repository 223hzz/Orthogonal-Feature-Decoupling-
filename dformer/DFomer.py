...
def build_dropout(dropout_layer):
    if dropout_layer is None or dropout_layer == 0:
        return nn.Identity()
    elif isinstance(dropout_layer, float):
        return nn.Dropout(dropout_layer)
    elif isinstance(dropout_layer, dict):
        if dropout_layer.get("type") == "DropPath":
            drop_prob = dropout_layer.get("drop_prob", 0.0)
            return DropPath(drop_prob)
        elif dropout_layer.get("type") == "Dropout":
            drop_prob = dropout_layer.get("drop_prob", 0.0)
            return nn.Dropout(drop_prob)
        else:
            raise ValueError(f"Unsupported dropout type: {dropout_layer.get('type')}")
    elif isinstance(dropout_layer, nn.Module):
        return dropout_layer
    else:
        raise ValueError(f"Unsupported dropout_layer type: {type(dropout_layer)}")
