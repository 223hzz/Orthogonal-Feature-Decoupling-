...
from nets.transfuse.visionTransformer import VisionTransformer
import math
...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.double_conv = nn.Sequential( #卷积后尺寸不变
...
class Up(nn.Module):
    ...
    def __init__(self, in_ch1, out_ch, in_ch2=0, attn=False):
        ...
        #使用双线性插值将输入特征图 x1 的尺寸放大 2 倍
        self.conv = DoubleConv(in_ch1+in_ch2, out_ch)
        #使用 DoubleConv 类对上采样后的特征图进行两次卷积操作，提取更丰富的特征。
        if attn:
            self.attn_block = Attention_block(in_ch1, in_ch2, out_ch)
        else:
            self.attn_block = None
        #使用 Attention_block 类实现注意力机制，将特征图 x1 和 x2 融合
...
def deit(pretrained=False, **kwargs):
    ...
    #第一个位置编码之外的所有位置编码，并将它们存储在pe变量中
    pe = pe.transpose(-1, -2)
    pe = pe.view(pe.shape[0], pe.shape[1], int(np.sqrt(pe.shape[2])), int(np.sqrt(pe.shape[2])))
    # print("pe.shape:",pe.shape)
    pe = F.interpolate(pe, size=(16, 16), mode='bilinear', align_corners=True)   #改形状以前是12*16
    pe = pe.flatten(2)
    pe = pe.transpose(-1, -2)
...
# model =  TransFuse_S(num_classes=2)
...
#     m = TransFuse_S(num_classes=2).to(device)
#     summary(m,(3, 256, 256))
...
# if __name__ == '__main__':
#     device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
#     model = TransFuse_S(num_classes=2).to(device)
...
# net = TransFuse_S(num_classes=2).to(device)
