...
def get_dwconv_layer(#创建深度可分离卷积
...
class MambaLayer(nn.Module):
    def __init__(self, input_dim, output_dim, d_state = 16, d_conv = 4, expand = 2):
        ...
        self.skip_scale= nn.Parameter(torch.ones(1))#一个可学习的参数
    ...
    def forward(self, x):
        ...
        n_tokens = x.shape[2:].numel()#返回的是n_tokens=H * W
        img_dims = x.shape[2:]#img_dims 就会是 (H, W)
...
class LightMUNet(nn.Module):
    ...
    def _make_down_layers(self):
        ...
        for i, item in enumerate(blocks_down):
            layer_in_channels = filters * 2**i#layer_in_channels = filters ,layer_in_channels = filters * 2,layer_in_channels = filters * 4,layer_in_channels = filters * 8
            downsample_mamba = (
                get_mamba_layer(spatial_dims, layer_in_channels // 2, layer_in_channels, stride=2)
                if i > 0
                else nn.Identity()
            )#当 i 等于0时，downsample_mamba 将是一个 nn.Identity() 实例
    ...
    def _make_up_layers(self):
        ...
        n_up = len(blocks_up)# 输出：3
        for i in range(n_up):
            sample_in_channels = filters * 2 ** (n_up - i)#filters*8,filters*4,filters*2,filters
            up_layers.append(
                nn.Sequential(
                    *[
    ...
    def decode(self, x: torch.Tensor, down_x: list[torch.Tensor]) -> torch.Tensor:
        for i, (up, upl) in enumerate(zip(self.up_samples, self.up_layers)):#[up:ResUpBlock | upl:get_conv_layer,get_upsample_layer]
            x = up(x) + down_x[i + 1]#
            x = upl(x)
