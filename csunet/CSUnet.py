...
class ConvSwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            concat_cov = self.up = nn.Sequential(
                Rearrange('b (h w) c -> b c h w', h=patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
...
#
#     # 1. 实例化模型
#     model =ConvSwinTransformerSys(num_classes=2, img_size=256).to(device)
#     model.eval()
#
#     # 2. 创建虚拟输入
