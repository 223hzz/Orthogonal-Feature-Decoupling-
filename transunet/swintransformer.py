...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.register_buffer("attn_mask", attn_mask) #导致维度不匹配,并且未在init（）中定义
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            #将两个不同来源的特征（如跳跃连接特征和上采样特征）拼接后投影到目标维度
            if i_layer == 0:
...
#
# # 过滤掉 attn_mask 参数
# pretrained_dict = {k: v for k, v in pretrained_dict.items() if 'attn_mask' not in k}
#
...
#
# model = SwinTransformerSys(img_size=256,num_classes=2,window_size=8)
...
#
#     # 1. 实例化模型
#     model =  SwinTransformerSys(num_classes=2).to(device)
#     model.eval()
#
#     # 2. 创建虚拟输入
