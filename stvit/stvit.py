...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=3, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 relative_pos=False, local=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, window_size=window_size, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                              proj_drop=drop, relative_pos=relative_pos)
    ...
    def forward(self, x):
        ...
        if self.local:
            ...
            attn = self.attn(x, x)
            attn = attn.view(B, self.H // self.window_size, self.W // self.window_size, self.window_size,
                             self.window_size, C)
...
class SemanticAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, multi_scale, window_size=7, window_sample_size=3, k_window_size=14, mlp_ratio=4.,
        ...
        self.attn = Attention(dim, num_heads=num_heads, window_size=None, qkv_bias=qkv_bias, attn_drop=attn_drop,
                              proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
    ...
    def forward(self, x, y=None):
        ...
        _, Hp, Wp, _ = x.shape
        num_samples = (
        Hp // self.window_size * self.window_sample_size, Wp // self.window_size * self.window_sample_size)
        if y == None:
            xx = x.reshape(B, Hp // self.window_size, self.window_size, Wp // self.window_size, self.window_size, C)
            windows = xx.permute(0, 1, 3, 2, 4, 5).contiguous().reshape(-1, self.window_size, self.window_size,
                                                                        C).permute(0, 3, 1, 2)
            ...
            y = y.reshape(B, num_samples[0] // self.window_sample_size, self.window_sample_size,
                          num_samples[1] // self.window_sample_size, self.window_sample_size, C)
        ...
        if self.pad_mask:
            ...
            pad_mask = F.unfold(pad_mask.unsqueeze(1), kernel_size=self.k_window_size, stride=self.window_size).view(B,
                                                                                                                     self.k_window_size,
                                                                                                                     self.k_window_size,
                                                                                                                     -1).permute(
                0, 3, 1, 2)
            ...
            pad_mask = None
        k_windows = F.unfold(xx.permute(0, 3, 1, 2), kernel_size=self.k_window_size, stride=self.window_size).view(B, C,
                                                                                                                   self.k_window_size,
                                                                                                                   self.k_window_size,
                                                                                                                   -1).permute(
            0, 4, 2, 3, 1)
        ...
        x = x.view(B, Hp // self.window_size, Wp // self.window_size, self.window_sample_size, self.window_sample_size,
                   C)
...
class RestoreBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, window_sample_size=3, k_window_size=27, mlp_ratio=4.,
                 qkv_bias=False, drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, layer_scale_init_value=1e-5,
                 pad_mask=True
                 ):
    ...
    def forward(self, x, y):
        ...
        if self.pad_mask:
            ...
            pad_mask[:, -pad_b:, -pad_r:] = -1000  # float('-inf')
            pad_mask = F.pad(pad_mask, (left, right, left, right), value=-1000)
            pad_mask = F.unfold(pad_mask.unsqueeze(1), kernel_size=self.k_window_size,
                                stride=self.window_sample_size).view(B, self.k_window_size, self.k_window_size,
                                                                     -1).permute(0, 3, 1, 2)
            pad_mask = pad_mask.reshape(-1, 1, self.k_window_size * self.k_window_size)
        ...
        k_windows = F.unfold(y.permute(0, 3, 1, 2), kernel_size=self.k_window_size,
                             stride=self.window_sample_size).view(B, C, self.k_window_size, self.k_window_size,
                                                                  -1).permute(0, 4, 2, 3, 1)
        k_windows = self.norm1(k_windows.reshape(B, -1, C)).reshape(-1, self.k_window_size * self.k_window_size, C)
...
class multi_scale_semantic_token1(nn.Module):
    ...
    def forward(self, x):
        B, C, _, _ = x.size()
        pool_x = F.adaptive_max_pool2d(x, (self.sample_window_size, self.sample_window_size)).view(B, C,
                                                                                                   self.num_samples).transpose(
            2, 1)
        return pool_x
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if upsample is not None:
            self.upsample = upsample(input_resolution=input_res, dim=dim,
                                     norm_layer=norm_layer)  # TODO: set input resolution x: B, H*W, C
        else:
            self.upsample = None
...
class Deit(nn.Module):
    ...
    def __init__(self, window_size, window_sample_size, k_window_size_1, k_window_size_2,
                 ...
                 downsample=None, upsample=None, input_res=(224, 224), relative_pos=False, use_conv_pos=False,
                 pad_mask=True):
        """
        Args:
        ...
        for i in range(depth):
            if i in [0, 6, 12]:
                ...
                self.blocks.append(SemanticAttentionBlock(
                    dim=embed_dim, window_size=window_size, window_sample_size=window_sample_size,
                    k_window_size=k_window_size_1,
                ...
                self.blocks.append(RestoreBlock(
                    dim=embed_dim, window_size=window_size, window_sample_size=window_sample_size,
                    k_window_size=restore_k_window_size,
                    num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, attn_drop=attn_drop_rate,
                    drop=drop_rate, drop_path=drop_path_rate[i], norm_layer=norm_layer, act_layer=act_layer,
                    pad_mask=pad_mask)
                )
            else:
                self.blocks.append(Block(
                    dim=embed_dim, window_size=window_sample_size, num_heads=num_heads, mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias, attn_drop=attn_drop_rate,
                    drop=drop_rate, drop_path=drop_path_rate[i], norm_layer=norm_layer, act_layer=act_layer,
                    relative_pos=relative_pos, local=bool(i % 2))
                )
        if downsample is not None:
        ...
        # patch expanding layer
        if upsample is not None:
            self.upsample = upsample(input_resolution=input_res, dim=embed_dim,
                                     norm_layer=norm_layer)  # TODO: set input resolution x: B, H*W, C
        else:
            self.upsample = None
    ...
    def __init__(self, input_resolution, num_samples, embed_dim=768, depth=6, num_heads=12, mlp_ratio=4., qkv_bias=True,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                window_size=(int(math.sqrt(self.num_samples)), int(math.sqrt(self.num_samples))), mlp_ratio=mlp_ratio,
        ...
        for i_layer in range(self.num_layers):
            if i_layer in [0, 1]:  # Swin Encoder layers
                layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),  # 96 192
                                   ...
                                   # No Downsampling in Basiclayer 3!
...
#     input = torch.rand((1, 3, 1024, 1024)).cuda(0)
...
#     model =  SemanticSTViT(num_classes=2).to(device)
#     model.eval()
#
#     # 2. 创建虚拟输入
