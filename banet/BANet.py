...
def rest_lite(pretrained=False, weight_path='',  **kwargs):
...
class DependencyPath(nn.Module):
    def __init__(self, weight_path=''):
        ...
        self.ResT = rest_lite(pretrained=False)
...
class BANet(nn.Module):
    def __init__(self, num_classes=2, weight_path=''):
...
#
#     # 1. 实例化模型
#     model = BANet(num_classes=2, weight_path=None).to(device)
#     model.eval()
#
#     # 2. 创建虚拟输入
